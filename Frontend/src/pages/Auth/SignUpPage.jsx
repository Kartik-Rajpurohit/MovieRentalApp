...
export default function SignUpPage() {
  ...
  const searchAddresses = async (event) => {
    if (!selectedCityId) { setAddressSuggestions([]); return; }
  ...
  const handleCountryChange = (countryId) => {
    ...
    setErrors((prev) => ({ ...prev, country: undefined, city: undefined, address: undefined }));
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh", background: "#f5f6fa" }}>
      <Card style={{ width: "460px", boxShadow: "0 4px 6px rgba(0,0,0,0.1)" }}>
        ...
        {errors.submit && (
          <Message severity="error" text={errors.submit} style={{ marginBottom: "16px" }} />
        )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          <div>
            ...
            <InputText value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder="First name" style={{ width: "100%" }} className={errors.firstName ? "p-invalid" : ""} />
          ...
          <div>
            ...
            <InputText value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder="Last name" style={{ width: "100%" }} className={errors.lastName ? "p-invalid" : ""} />
          ...
          <div>
            ...
            <InputText value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Enter your email" type="email" style={{ width: "100%" }} className={errors.email ? "p-invalid" : ""} />
          ...
          <div>
            ...
            <Password value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Create a password" toggleMask feedback={false} style={{ width: "100%" }} inputStyle={{ width: "100%" }} className={errors.password ? "p-invalid" : ""} />
            {errors.password && <small className="p-error">{errors.password}</small>}
          </div>

          {/* Divider */}
          <div style={{ borderTop: "1px solid #e5e7eb", paddingTop: "8px" }}>
            <p style={{ margin: "0 0 4px 0", fontSize: "13px", fontWeight: 600, color: "#374151" }}>Address</p>
          </div>
          ...
          <div>
            ...
            <Dropdown
              ...
              placeholder={!selectedCountryId ? "Select country first" : citiesLoading ? "Loading..." : "Select city"}
              style={{ width: "100%" }}
          ...
          <div>
            ...
            <AutoComplete
              ...
              placeholder={!selectedCityId ? "Select city first" : "Type or select address"}
            ...
            {selectedCityId && !selectedAddressId && (
              <small style={{ color: "#6b7280" }}>Type a new address or select from suggestions</small>
            )}
          </div>

          {/* District */}
          <div>
            <label style={labelStyle}>District <span style={{ color: "#9ca3af", fontWeight: 400 }}>(optional)</span></label>
            <InputText value={district} onChange={(e) => setDistrict(e.target.value)} placeholder="District" style={{ width: "100%" }} />
          </div>

          {/* Postal Code */}
          <div>
            <label style={labelStyle}>Postal Code <span style={{ color: "#9ca3af", fontWeight: 400 }}>(optional)</span></label>
            <InputText value={postalCode} onChange={(e) => setPostalCode(e.target.value)} placeholder="Postal code" style={{ width: "100%" }} />
          </div>
          ...
          <Button label="Sign Up" onClick={handleSignUp} loading={loading} style={{ width: "100%" }} />

          <div style={{ textAlign: "center", fontSize: "14px", color: "#6b7280" }}>
            Already have an account?{" "}
            <span onClick={() => navigate("/login")} style={{ cursor: "pointer", color: "#6366f1", fontWeight: 600 }}>
              Sign In
            </span>
          </div>
        </div>
